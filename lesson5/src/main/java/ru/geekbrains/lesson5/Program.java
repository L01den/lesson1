...
public class Program {
    ...
    public static void main(String[] args) {
        ...
        while (f){
            ...
            System.out.println("9. Добавить текстуру модели");
                    ...
                    switch (no) {
                        ...
                        case 9:
                            ...
                            if (scanner.hasNextInt()){
                                int modelNo = scanner.nextInt();
                                scanner.nextLine();
                                editor3D.setTextureInModel(modelNo);
                            }
                            else {
                                System.out.println("Номер модели указан некорректно.");
                            }
...
//    Удаление модели целиком
    public void deleteModel(int no){
        // Предусловие
        checkProjectFile();

        ArrayList<Model3D> models = (ArrayList<Model3D>)businessLogicalLayer.getAllModels();
        if (no < 0 || no > models.size() - 1)
            throw new RuntimeException("Номер модели указан некорректною.");
        businessLogicalLayer.deleteModel(models.get(no));
        System.out.println("Модель удалена");
    }

//    Добавление текстуры для выбранной модели

    public void setTextureInModel(int no){
        // Предусловие
        checkProjectFile();
        ...
        if (no < 0 || no > models.size() - 1)
            throw new RuntimeException("Номер модели указан некорректною.");
        businessLogicalLayer.setTextureInModel(models.get(no));
...
class EditorBusinessLogicalLayer implements BusinessLogicalLayer{
    ...
    @Override
    public void setTextureInModel(Model3D model) {
...
interface BusinessLogicalLayer{
    ...
    void setTextureInModel(Model3D model);
}
...
//    public Texture getOneTexture(int no){
//        return textures.ge
//    }
